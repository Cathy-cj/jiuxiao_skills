...
<课节编码>-upgrade/<任意名>-upgrade.md   # 晋级赛；旧名 -quiz / *-quiz.md 仅当 -upgrade 不存在时可读
<课节编码>-homework/<任意名>-homework.md
```

例如 `W1-L6-1-upgrade`（或旧名 `W1-L6-1-quiz`）归属于 `W1-L6-1`，即使 Markdown 文件名或文内“课件 ID”采用另一套前缀。不得因为目录叫 upgrade 而当成没有晋级赛。
...
- 上述目录名规则只适用于 `lesson_data`。`week_question_data.courseware_num` 必须逐字使用 upgrade Markdown 的 `## 课件 ID`，不能根据目录名重建或纠正。
...
| `week_question_data` | 当前课节 `*-upgrade/` 内的 `*-upgrade.md`；无 upgrade 时才读旧名 `*-quiz/` 内的 `*-quiz.md` |
